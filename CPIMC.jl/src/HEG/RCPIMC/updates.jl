const ex_radius = 2 # maximum radius for excitations
function move_particle(c::Configuration, e::Ensemble) :: Tuple{Float64, Step}
    ...
    # if there are no empty non-interacting orbitals in neighbourhood: make no change (return empty Step())
    if length(oe) == 0
        return 1.0, Step()
    end
    y = rand(oe)
...
function add_type_B(c::Configuration, e::Ensemble) :: Tuple{Float64, Step}
    ...
    # See which of the two imiginary time points is the "left border of the interval".
    # If there are no kinks affecting any of the new kink orbitals:
    # then any of the two τ's can be firstτ
    if borders[2] == 1
        if rand() < 0.5
            firstτ = τ1
            lastτ = τ2
        ...
        prop_prob *= 0.5
    # If there are kinks affecting any of the new kink orbitals:
    # which τ is firstτ depends on which τ is the left one inside the given interval
    elseif τ1 > borders[1]
        if τ2 > borders[1]
            firstτ = min(τ1,τ2)
            lastτ = max(τ1,τ2)
    ...
    # We do consider states that differ only in the order of indices of kinks
