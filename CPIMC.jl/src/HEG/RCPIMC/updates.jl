...
function move_particle(c::Configuration, e::Ensemble) :: Tuple{Float64, Step}
    ...
    #println("kindiff:", e.β*(get_energy(y)-get_energy(x)))# debug information
...
function add_type_B(c::Configuration, e::Ensemble) :: Tuple{Float64, Step}
    ...
    # If there are no kinks affecting any of the new kinks orbitals then any
    # of the two τ's can be firstτ
    if borders[2] == 1
        if rand() < 0.5
            firstτ = τ1
            lastτ = τ2
