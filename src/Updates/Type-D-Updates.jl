...
function remove_type_D(c::Configuration, e::Ensemble) :: Tuple{Float64,Step}
    ...
    if rand() > 0.5
        ...
        opportunities_reverse_new_orb1 = intersect!(get_sphere_with_same_spin(apply_step(c,Δ).kinks[changed_kink_τ].i, dk = ex_radius), occs)
        ...
        delta_di = get_change_diagonal_interaction(apply_step(c,Δ), e, T4(apply_step(c,Δ).kinks[changed_kink_τ].i,apply_step(c,Δ).kinks[changed_kink_τ].j, removed_orb1,removed_orb2), removed_kink_τ, changed_kink_τ)
        ...
        dw = (1.0/e.β)* (1.0/dw_off_diag) * exp(e.β * delta_τ * (get_energy(apply_step(c,Δ).kinks[changed_kink_τ].i) + get_energy(apply_step(c,Δ).kinks[changed_kink_τ].j) -
        ...
        #calculate reverse_prop_prob
        occs = occupations(apply_step(c,Δ), changed_kink_τ)
        opportunities_reverse_new_orb1 = intersect!(get_sphere_with_same_spin(apply_step(c,Δ).kinks[changed_kink_τ].k, dk = ex_radius), occs)
        delete!(opportunities_reverse_new_orb1, apply_step(c,Δ).kinks[changed_kink_τ].i)
        delete!(opportunities_reverse_new_orb1, apply_step(c,Δ).kinks[changed_kink_τ].j)
        @assert(in(removed_orb1,opportunities_reverse_new_orb1))
        ...
        #calculate weight change
        delta_di = get_change_diagonal_interaction(apply_step(c,Δ), e, T4(apply_step(c,Δ).kinks[changed_kink_τ].k,apply_step(c,Δ).kinks[changed_kink_τ].l, removed_orb1,removed_orb2), changed_kink_τ, removed_kink_τ)
        ...
        dw = (1.0/e.β)*(1.0/dw_off_diag) * exp(e.β * delta_τ*(get_energy(apply_step(c,Δ).kinks[changed_kink_τ].k) + get_energy(apply_step(c,Δ).kinks[changed_kink_τ].l) -
                                                            get_energy(removed_orb1) - get_energy(removed_orb2)) + delta_di)
