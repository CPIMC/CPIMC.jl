...
function add_type_D(c::Configuration, e::Ensemble) :: Tuple{Float64,Step}
    ...
    if rand() >= 0.5
        ...
        delta_di = ΔWdiag_element(c, e, last(old_kink).i,last(old_kink).j, new_orb1, new_orb2, τ_new_kink, first(old_kink))
        ...
        dw_off_diag = abs(Woffdiag_element(e,apply_step(c,Δ).kinks[τ_new_kink])) * abs(Woffdiag_element(e,apply_step(c,Δ).kinks[first(old_kink)])) /
                                                abs(Woffdiag_element(e,last(old_kink)))
        dw = e.β * dw_off_diag* exp(-(e.β * delta_τ*(energy(last(old_kink).i) + energy(last(old_kink).j)-
                                                         energy(new_orb1) - energy(new_orb2)) + e.β * delta_di))
        ...
        delta_di = ΔWdiag_element(c, e, last(old_kink).k, last(old_kink).l, new_orb1, new_orb2, first(old_kink), τ_new_kink)
        ...
        dw = e.β * dw_off_diag* exp(-(e.β * delta_τ*(energy(last(old_kink).k) + energy(last(old_kink).l) -
                                                         energy(new_orb1) - energy(new_orb2)) + e.β * delta_di))
...
function remove_type_D(c::Configuration, e::Ensemble) :: Tuple{Float64,Step}
    ...
    if rand() > 0.5
        ...
        delta_di = ΔWdiag_element(apply_step(c,Δ), e, apply_step(c,Δ).kinks[changed_kink_τ].i,apply_step(c,Δ).kinks[changed_kink_τ].j, removed_orb1,removed_orb2, removed_kink_τ, changed_kink_τ)

        dw_off_diag = abs(Woffdiag_element(e,T4(removed_orb1, removed_orb2, apply_step(c,Δ).kinks[changed_kink_τ].k, apply_step(c,Δ).kinks[changed_kink_τ].l))) *
                        abs(Woffdiag_element(e,T4(apply_step(c,Δ).kinks[changed_kink_τ].i, apply_step(c,Δ).kinks[changed_kink_τ].j, removed_orb1, removed_orb2))) /
                            abs(Woffdiag_element(e,apply_step(c,Δ).kinks[changed_kink_τ]))

        delta_τ = Float64(changed_kink_τ - removed_kink_τ)
        if delta_τ < 0
            delta_τ +=1
        end
        dw = (1.0/e.β)* (1.0/dw_off_diag) * exp(e.β * delta_τ * (energy(apply_step(c,Δ).kinks[changed_kink_τ].i) + energy(apply_step(c,Δ).kinks[changed_kink_τ].j) -
                                                                    energy(removed_orb1) - energy(removed_orb2)) + e.β * delta_di)
        ...
        #calculate weight change
        delta_di = ΔWdiag_element(apply_step(c,Δ), e, apply_step(c,Δ).kinks[changed_kink_τ].k,apply_step(c,Δ).kinks[changed_kink_τ].l, removed_orb1,removed_orb2, changed_kink_τ, removed_kink_τ)
        ...
        dw = (1.0/e.β)*(1.0/dw_off_diag) * exp(e.β * delta_τ*(energy(apply_step(c,Δ).kinks[changed_kink_τ].k) + energy(apply_step(c,Δ).kinks[changed_kink_τ].l) -
                                                            energy(removed_orb1) - energy(removed_orb2)) + e.β * delta_di)
