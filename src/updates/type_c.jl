...
function add_type_C(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64,Step}
    ...
    if rand() >= 0.5
        #add kink left
        opportunities_new_orb1 = possible_new_orb1_C(occs, old_kink.k, old_kink.l,old_kink.i, old_kink.j)
        ...
        τ_interval = old_kink_τ - τ_prev_affecting( c.kinks, Set([old_kink.k, old_kink.l, new_orb1, new_orb2]), old_kink_τ )
        ...
        if τ_new_kink > old_kink_τ  #new kink was added left of old kink
            drop_orbs = (old_kink.k, old_kink.l)
            add_orbs = (new_orb1, new_orb2)
        else
            drop_orbs = nothing
            add_orbs = nothing
        end
        ...
        add_kinks = (
                    ...
                    old_kink_τ => T4( old_kink.i, old_kink.j, random_shuffle(new_orb2, new_orb1 )... )
                    )
        ...
        delta_di = ΔWdiag_element(m, e, c, new_orb1, new_orb2, old_kink.k, old_kink.l, τ_new_kink, old_kink_τ)
        dw_off_diag = abs( Woffdiag_element(m, e, new_orb1, new_orb2, old_kink.k, old_kink.l) * Woffdiag_element(m, e, last(last(add_kinks))) / Woffdiag_element(m, e, old_kink) )
        dw = e.β * dw_off_diag * exp(-(e.β * delta_τ*(energy(m, new_orb1) + energy(m, new_orb2) - energy(m, old_kink.k) - energy(m, old_kink.l)) + e.β * delta_di))
        ...
        # add kink right
        opportunities_new_orb1 = possible_new_orb1_C(occs, old_kink.i, old_kink.j, old_kink.k, old_kink.l)
        if isempty(opportunities_new_orb1)
            @assert(false)
            return 1.0, Step()
        end
        ...
        @assert !in(new_orb2, occs) & (new_orb1 != new_orb2)
        τ_interval = τ_next_affecting( c.kinks , Set([old_kink.i, old_kink.j, new_orb1, new_orb2]), old_kink_τ ) - old_kink_τ
        ...
        # see if c.occupations change
        if τ_new_kink < old_kink_τ  #new kink was added right of old kink
            drop_orbs = (old_kink.i,old_kink.j)
            add_orbs = (new_orb1,new_orb2)
        else
            drop_orbs = nothing
            add_orbs = nothing
        ...
        add_kinks =(
                    ...
                    # shuffle creators
                    old_kink_τ => T4( random_shuffle( new_orb2, new_orb1)... , old_kink.k, old_kink.l )
                    )
        drop_kinks = (old_kink_τ => old_kink,)
        ...
        # weight factor
        delta_di = ΔWdiag_element(m, e, c, new_orb1, new_orb2, old_kink.i, old_kink.j, old_kink_τ, τ_new_kink)
        dw_off_diag = abs( Woffdiag_element(m, e, old_kink.i, old_kink.j, new_orb1, new_orb2) * Woffdiag_element(m, e, last(last(add_kinks))) / Woffdiag_element(m, e, old_kink) )
        dw = e.β * dw_off_diag* exp(-(e.β * delta_τ*(energy(m, new_orb1) + energy(m, new_orb2) - energy(m, old_kink.i) - energy(m, old_kink.j)) + e.β * delta_di))
