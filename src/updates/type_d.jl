...
function add_type_D(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64,Step}
    ...
    if rand() >= 0.5
        ...
        dw_off_diag = abs(Woffdiag_element(m, e, last(old_kink).i, last(old_kink).j,new_orb1, new_orb2) * Woffdiag_element(m, e, last(last(add_kinks))) /
        ...
        dw_off_diag = abs( Woffdiag_element(m, e, last(first(add_kinks))) * Woffdiag_element(m, e, last(last(add_kinks))) / Woffdiag_element(m, e, last(old_kink)) )
...
function remove_type_D(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64,Step}
    ...
    if rand() > 0.5
        ...
        inverse_prop_prob = (0.5/(length(c.kinks)-1)) * (1.0/length(opportunities_reverse_new_orb1)) * (1.0/Float64(τ_Intervall)) * (1/2)# TODO: use float() ?

        # calculate weight change
        delta_di = ΔWdiag_element(m, e, apply_step(c,Δ), last(first(add_kinks)).i, last(first(add_kinks)).j, removed_orb1,removed_orb2, removed_kink_τ, changed_kink_τ)# TODO: use the references from c.kink[...] as given by add_kinks

        dw_off_diag = abs( Woffdiag_element(m, e, removed_orb1, removed_orb2, last(first(add_kinks)).k, last(first(add_kinks)).l) * Woffdiag_element(m, e, last(first(add_kinks)).i, last(first(add_kinks)).j, removed_orb1, removed_orb2) / Woffdiag_element(m, e, last(first(add_kinks))) )# TODO: use the references from c.kink[...] as given by add_kinks
        ...
        #calculate weight change
        delta_di = ΔWdiag_element(m, e, apply_step(c,Δ),last(first(add_kinks)).k, last(first(add_kinks)).l, removed_orb1,removed_orb2, changed_kink_τ, removed_kink_τ)
