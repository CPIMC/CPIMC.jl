...
function add_type_D(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64,Step}
    ...
    if rand() >= 0.5
        ...
        if τ_new_kink > old_τ  #new kink was added left of old kink
            ...
            add_orbs = Set([old_kink.i, old_kink.j])
        else
        ...
        dw_off_diag = abs(Woffdiag_element(m, e, old_kink.i, old_kink.j, new_orb1, new_orb2) * Woffdiag_element(m, e, last(last(add_kinks))) /
        ...
        if τ_new_kink < old_τ  #new kink was added right of old kink
            drop_orbs = Set([new_orb1, new_orb2])
            add_orbs = Set([old_kink.k, old_kink.l])
        else
            drop_orbs = Set{basis(c)}()
            add_orbs = Set{basis(c)}()
        end
...
function remove_type_D(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64,Step}
    ...
    if rand() > 0.5
        ...
        drop_kinks = (removed_kink_τ => removed_kink, changed_kink_τ => changed_kink, )
        ...
        τ_interval = changed_kink_τ - τ_prev_affecting( new_c.kinks, Set([removed_orb1, removed_orb2, last(first(add_kinks)).i, last(first(add_kinks)).j]), changed_kink_τ )
        ...
        # calculate weight change
        delta_di = ΔWdiag_element(m, e, new_c, last(first(add_kinks)).i, last(first(add_kinks)).j, removed_orb1, removed_orb2, removed_kink_τ, changed_kink_τ)

        dw_off_diag = abs( Woffdiag_element(m, e, removed_orb1, removed_orb2, last(first(add_kinks)).k, last(first(add_kinks)).l) * Woffdiag_element(m, e, last(first(add_kinks)).i, last(first(add_kinks)).j, removed_orb1, removed_orb2) / Woffdiag_element(m, e, last(first(add_kinks))) )
        ...
        dw = (1.0/e.β)* (1.0/dw_off_diag) * exp(e.β * delta_τ * (energy(m, last(first(add_kinks)).i) + energy(m, last(first(add_kinks)).j) - energy(m,removed_orb1) - energy(m,removed_orb2)) + e.β * delta_di)# TODO: use the references from c.kink[...] as given by add_kinks
        ...
        @assert(in(removed_orb1, opportunities_reverse_new_orb1))
        τ_interval =  τ_next_affecting( new_c.kinks, Set([ last(first(add_kinks)).k, last(first(add_kinks)).l, removed_orb1, removed_orb2]), changed_kink_τ ) - changed_kink_τ
        if τ_interval < 0
            τ_interval +=1
        end
        ...
        #calculate weight change
        delta_di = ΔWdiag_element(m, e, new_c, last(first(add_kinks)).k, last(first(add_kinks)).l, removed_orb1, removed_orb2, changed_kink_τ, removed_kink_τ)
        ...
        dw = (1.0/e.β)*(1.0/dw_off_diag) * exp(e.β * delta_τ*(energy(m, last(first(add_kinks)).k) + energy(m, last(first(add_kinks)).l) - energy(m, removed_orb1) - energy(m, removed_orb2)) + e.β * delta_di)
