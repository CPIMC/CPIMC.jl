...
function add_type_E(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64,Step}
    ...
    if rand() >= 0.5
        ...
        prop_prob *= 1.0/16.0
        add_kink1 = first(old_kink) => T4( shuffle_indices( new_kink_new_annihilator, changed_kink_old_creator, changed_kink_old_annihilator, new_kink_new_creator )... )
        #shuffle new kink
        add_kink2 = τ_new_kink => T4( shuffle_indices( new_kink_new_creator, new_kink_old_creator, new_kink_old_annihilator, new_kink_new_annihilator )... )
...
function remove_type_E(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64,Step}
    ...
    if rand() > 0.5
        ...
        #shuffle Indices
        prop_prob *= 0.25
        add_kink = changed_kink_τ => T4( shuffle_indices( changed_kink_old.i, removed_kink.i, changed_kink_old.l, removed_kink.l )... )
