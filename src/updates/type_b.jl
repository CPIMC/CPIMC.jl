...
function add_type_B(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64, Step}
    ...
    orb_c = rand(occs)
    prop_prob *= 1.0/e.N
    orb_d = rand(drop_orbs(occs, (orb_c,)))
    prop_prob *= 1.0/(e.N-1)
    ...
    if firstτ > lastτ
        orbs_drop = (orb_c, orb_d)
        orbs_add = (orb_a, orb_b)
    else
        orbs_drop = nothing
        orbs_add = nothing
    end
    ...
    Δ = Step(orbs_drop, nothing, orbs_add, kinks_add)
    ...
    dv = ( 1.0/ length( right_type_B_removable_pairs(c.kinks) ) ) / prop_prob
...
function remove_type_B(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64, Step}
    ...
    if first(kink1) > first(kink2)
        ...
        orbs_add = (last(kink2).i, last(kink2).j)
        orbs_drop = (last(kink2).k, last(kink2).l)
    ...
    # MC step generated by this update
    Δ = Step(orbs_drop, (kink1, kink2), orbs_add, nothing)
    @assert(delta_τ > 0)
    @assert(delta_τ <= 1)
    ...
    borders = τ_borders(c.kinks, ijkl, first(kink1))
    possible_τ2_interval = borders[2]-borders[1]
    if possible_τ2_interval < 0
        possible_τ2_interval = 1 + possible_τ2_interval
    end
    occs_τ_kink1 = occupations_at(c, first(kink1))
    occs_τ_kink2 = occupations_at(c, first(kink2))
    orb_a = last(kink1).i
    orb_b = last(kink1).j
    orb_c = last(kink1).k
    orb_d = last(kink1).l
    ...
    # calculate change in diagonal interaction energy
    delta_di = ΔWdiag_element(m, e, c, last(kink1).i, last(kink1).j, last(kink1).k, last(kink1).l, first(kink1), first(kink2))
    # weight factor
...
function change_type_B(m::Model, e::Ensemble, c::Configuration) :: Tuple{Float64,Step} #This update is redundant wif we have add- and remove-Type-C-Updates
    ...
    kink_opportunities_reverse = right_type_B_pairs(c.kinks)
    opportunities_reverse = filter( x -> isunaffected_in_interval(c.kinks, x, first(kink1), first(kink2))
