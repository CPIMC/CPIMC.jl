...
function ΔW_diag(m::Model, i, j, k, l, occ)
    ...
    Δ = sum( wminus(m, ν,k,k,ν) + wminus(m, ν,l,l,ν) for ν in drop_orbs(occ, [k,l]) )# interactions of mean field with k and l
    ...
    Δ -= sum( wminus(m, ν,i,i,ν) + wminus(m, ν,j,j,ν) for ν in drop_orbs(occ, [k,l]) )# interactions of mean field with i and j
...
function ΔW_diag(m::Model, i, j, occ)
    ...
    # contributions due to mean field interactions of the annihilated orbitals
    Δ = sum( wminus(m, ν,j,j,ν) for ν in drop_orbs(occ, (j,)) )# interactions of mean field with k
    # contributions due to mean field interactions of the created orbitals
    Δ -= sum( wminus(m, ν,i,i,ν) for ν in drop_orbs(occ, (j,)) )
end
